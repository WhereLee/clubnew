-- =============================================
-- ⚠ 已废弃：schema 已迁移到 Flyway（src/main/resources/db/migration/）
--   本文件仅作历史存档，请勿再手工执行。
--   初始化数据库：建库后由 Flyway 自动执行 V1/V2 迁移。
-- =============================================
-- 社团全流程管理系统 · 全量建表 + 种子数据
...
CREATE TABLE recruit (
    ...
    remark         VARCHAR(500)  DEFAULT NULL COMMENT '备注',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='纳新活动表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='纳新报名记录表';
...
CREATE TABLE fund (
    ...
    remark        VARCHAR(500)  DEFAULT NULL COMMENT '备注',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='经费申请表';
